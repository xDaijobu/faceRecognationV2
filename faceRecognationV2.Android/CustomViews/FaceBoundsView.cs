...
namespace faceRecognationV2.Droid.CustomViews
{
    public class FaceBoundsView : View
    {
        ...
        #region Versi 1 ( langsung draw si Faces[] )
        ...
        #endregion
        #region Versi 2
        private List<Recognition> mappedRecognitions;

        public void OnFacesDetected(List<Recognition> recognitions,
                                    int textureWidth,
                                    int textureHeight,
                                    int previewImageWidth,
                                    int previewImageHeight,
                                    int sensorOrientation)
        {
            ...
            _widthRatio = (double)textureHeight / previewImageWidth;
            _heightRatio = (double)textureWidth / previewImageHeight;
            mappedRecognitions = recognitions;
            Invalidate();
        }
        ...
        protected override void OnDraw(Canvas canvas)
        {
            ...
            {
                foreach(var recognition in mappedRecognitions)
                {
                    Rect rect = null;
                    if (_sensorOrientation == 90)
                        rect = new Rect((int)((_previewImageHeight - recognition.Location.Top) * _heightRatio),
                                        (int)(recognition.Location.Left * _widthRatio),
                                        (int)((_previewImageHeight - recognition.Location.Bottom) * _heightRatio),
                                        (int)(recognition.Location.Right * _widthRatio));
                    else if (_sensorOrientation == 270)
                        rect = new Rect((int)((_previewImageHeight - recognition.Location.Top) * _heightRatio),
                                        (int)((_previewImageWidth - recognition.Location.Left) * _widthRatio),
                                        (int)((_previewImageHeight - recognition.Location.Bottom) * _heightRatio),
                                        (int)((_previewImageWidth - recognition.Location.Right) * _widthRatio));
